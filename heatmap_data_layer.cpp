...
//      "new_height and new_width to be set at the same time.";
  ...
  // Read the file with filenames and labels
    CHECK(new_height>=crop_size && new_width>=crop_size) << "Both new_width and new_height must greater than crop_size.";
...
//    lines_.push_back(std::make_pair(line.substr(0, pos), label));
      ...
      CHECK_EQ((int)line_split.size()-1,points_num)<<"points num is not same with source.";
...
template <typename Dtype>
void HeatmapDataLayer<Dtype>::transform_data_label(Blob<Dtype>* data_blob,Blob<Dtype>* label_blob,const cv::Mat& img,
                                            float img_width_scale, float img_height_scale)
{
    ...
    const bool rotation=this->layer_param_.heatmap_data_param().rotation();
...
//    const bool has_mean_file = this->layer_param_.heatmap_data_param().has_mean_file();
    ...
    if (has_mean_values)
    {
        CHECK(mean_values.size() == 1 || mean_values.size() == img_channels) << "Specify either 1 mean_value or as many as channels: " << img_channels;
...
//        angle =90;
    ...
    //label处理
    //随机剪裁
    size_t pts_size=lines_[lines_id_].size()-1;
    vector<cv::Point> points(pts_size);
    for(int i=0;i<pts_size;++i)
    {
        vector<string> numstr_split;
        boost::split(numstr_split,lines_[lines_id_][i+1],boost::is_any_of(","),boost::token_compress_on);
        CHECK_EQ(numstr_split.size(),2)<<"coordinate param has two numbers.";
        int x= (int)roundf(atoi(numstr_split[0].c_str())*img_width_scale)-w_off;
        int y= (int)roundf(atoi(numstr_split[1].c_str())*img_height_scale)-h_off;
//        x = x>=0 ? x : -1;
//        y = y>=0 ? y : -1;
        points[i].x=x;
        points[i].y=y;
    ...
    if(rotation)
    {
        float center_x=(w_off+width)/2.0f;
        float center_y=(h_off+height)/2.0f;
        for(int i=0;i<pts_size;++i)
        {
            float r_x=points[i].x-center_x;
            float r_y=points[i].y-center_y;
            int x_rotated= roundf(center_x+r_x*std::cos(arc_angle)-r_y*std::sin(arc_angle));
            int y_rotated= roundf(center_y+r_y*std::cos(arc_angle)+r_x*std::sin(arc_angle));
...
//            }
    ...
    for(int i=0;i<pts_size;++i)
    {
        if(hm_pts[i].x<0 || hm_pts[i].y<0)
            continue;
        hm_pts[i].x=roundf(hm_width_scale*hm_pts[i].x);
        hm_pts[i].y=roundf(hm_height_scale*hm_pts[i].y);
    }
...
//    float label_resize_fact = (float) label_height / (float) outsize;
    ...
    for (int idx_ch = 0; idx_ch < label_num_channels; idx_ch++)
    {
        ...
        for (int i = 0; i < label_height; i++)
        {
            ...
            {
                // 计算索引
                int label_idx = idx_ch * label_channel_size + i * label_height + j;
                float gaussian = ( 1 / ( sigma * sqrt(2 * M_PI) ) ) * exp( -0.5 * ( pow(i - hm_pts[idx_ch].y, 2.0) + pow(j - hm_pts[idx_ch].x, 2.0) ) * pow(1 / sigma, 2.0) );
                gaussian = 4 * gaussian;
                //for debug
